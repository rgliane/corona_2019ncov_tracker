...
class RecoveredScreen extends StatefulWidget {
  static String id = 'recovered_screen';

  @override
  _RecoveredScreenState createState() => _RecoveredScreenState();
}
...
class _RecoveredScreenState extends State<RecoveredScreen> {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          title: Text('Recovered Breakdown'),
        ),
        body: Container(
          child: FutureBuilder(
            future: getAllItems(),
                      ...
                      child: Padding(
                        ...
                        child: Card(
                          child: Column(
                            children: <Widget>[
                              ListTile(
                                title: Text(snapshot.data.totalRecovered.toString() + ' - Total Recovered', style: TextStyle(fontSize: 20.0),),
                                subtitle: Text('Recovered breakdown by Country, State/Province'),
                              ),
                            ],
                          ),
                        ),
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          return Padding(
                            ...
                            child: Card(
                              child: ListTile(
                                title: Text(snapshot.data.getAttributes[index].recovered.toString() + ' - ' + snapshot.data.getAttributes[index].countryRegion + ', ' + snapshot.data.getAttributes[index].provinceState, style: TextStyle(fontSize: 19.0),),
                              ),
                            ),
                          );
//                          return ListTile(
//                            title: Text(snapshot
//                                .data.getAttributes[index].countryRegion),
//                          );
                        },
                      ),
                    )),
