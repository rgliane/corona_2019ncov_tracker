...
class DeathsScreen extends StatefulWidget {
  static String id = 'deaths_screen';
...
class _DeathsScreenState extends State<DeathsScreen> {
  ...
  Future<AttributesData> getAllItems() async {
    ...
    http.Response response = await http.get(
        'https://services1.arcgis.com/0MSEUqKaxRlEPj5g/arcgis/rest/services/ncov_cases/FeatureServer/1/query?f=json&where=Deaths%3E0&returnGeometry=false&spatialRel=esriSpatialRelIntersects&outFields=*&orderByFields=Deaths%20desc%2CCountry_Region%20asc%2CProvince_State%20asc&outSR=102100&resultOffset=0&resultRecordCount=250&cacheHint=true');
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          title: Text('Deaths Breakdown'),
        ),
        body: Container(
          child: FutureBuilder(
            future: getAllItems(),
            builder: (BuildContext context, AsyncSnapshot<AttributesData> snapshot) {
                          ...
                          child: Column(
                            children: <Widget>[
                              ListTile(
                                ...
                                subtitle: Text(
                                    'Deaths breakdown by Country, State/Province'),
                              ),
                            ],
                          ),
                        ),
                            ...
                            itemBuilder: (context, index) {
                              return Padding(
                                ...
                                child: Card(
                                  child: ListTile(
                                    title: Text(
                                      snapshot.data.getAttributes[index].deaths
                                          .toString() +
                                          ' - ' +
